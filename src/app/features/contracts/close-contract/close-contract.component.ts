...
export class CloseContractComponent implements OnInit {
  ...
  readonly liq = computed(() => {
    ...
    const ret =
      this.retDate && this.retTime ? new Date(`${this.retDate}T${this.retTime}:00`) : null;
    ...
    const subtCoverage = (c.contractCoverages ?? []).reduce(
      (s, cc) => s + Number(cc.coverage.price_per_day) * days,
      0,
    );
    ...
    console.log({ subtTarifa });
    console.log({ subtCoverage });
    console.log({ subtComb });
    ...
    const kmDriven =
      retKmN !== null && c.delivery.departure_km !== undefined
    ...
    return {
      ...
      grandTotal,
      kmDriven,
      dedCount: this.deductions.filter((d) => d.active).length,
    };
  });

  /* ── Helpers visuales ───────────────────────────────────── */
  fuelSegActive(i: number) {
    return i <= this.retFuel();
  }
  setFuel(i: number) {
    this.retFuel.set(i);
  }
  ...
  toggleDed(i: number) {
    this.deductions[i].active = !this.deductions[i].active;
  }
